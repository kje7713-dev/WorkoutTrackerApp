...
struct BlockListView: View {
                ...
                if blocks.isEmpty {
                    ...
                    ForEach(blocks) { block in
                        NavigationLink(block.name) {
                            // Inline, minimal detail view so we don't depend on other types
                            VStack(spacing: 16) {
                                Text(block.name)
                                    .font(.largeTitle)
                                    .fontWeight(.bold)
                                Text("Weeks: \(block.weeksCount)")
    ...
    private func addTestBlock() {
        let block = BlockTemplate(
            name: "DBG \(Date().formatted(date: .abbreviated, time: .shortened))",
            weeksCount: 4
        )
        context.insert(block)
        do {
            ...
            print("🔥 Failed to save test block: \(error)")
        }
    }
}
struct BlockBuilderView: View {
    ...
    // MARK: - Actions
        ...
        do {
            try context.save()
            print("✅ Saved block \(block.name) with id \(block.id)")
            isGenerating = false
            dismiss()
        } catch {
            print("🔥 Failed to save block: \(error)")
            errorMessage = "Failed to save block: \(error.localizedDescription)"
            isGenerating = false
        }
    }
}
