...
struct AutoProgramService {
    ...
    static func generateBlock(
        ...
        func template(for name: String, category: String?) -> ExerciseTemplate {
            ...
            // Try to find existing template in store (simple fetch, no #Predicate)
            let descriptor = FetchDescriptor<ExerciseTemplate>()
            let fetched = try? context.fetch(descriptor)
            let existing = fetched?.first(where: { $0.name == name })
            if let existing {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if points.isEmpty {
                ...
                // ✅ Back to the original pattern, but the heavy bit is now in MetricRow
                ForEach(Array(points.enumerated()), id: \.0) { (_, point) in
...
private func generateAIBlock() {
        ...
        // Try to parse TMs, but if they fail we'll still create a basic block
        ...
        // Base config for auto-programming if TMs are valid
        let config = AutoProgramConfig(
            ...
            trainingMaxes: {
                ...
                if let s = squat { dict["Back Squat"] = s }
                if let b = bench { dict["Bench Press"] = b }
                if let d = dead  { dict["Deadlift"] = d }
                return dict
            }()
        )

        do {
            // If training maxes are all present and valid, try full auto-program
            if squat != nil, bench != nil, dead != nil {
                _ = try AutoProgramService.generateBlock(in: context, config: config)
                print("✅ AutoProgramService.generateBlock succeeded")
            } else {
                // Fallback: simple empty block with just metadata
                let fallback = BlockTemplate(
                    name: trimmedName,
                    weeksCount: weeks,
                    createdAt: .now,
                    createdByUser: true,
                    notes: "Fallback block (invalid training maxes)"
                )
                context.insert(fallback)
                try context.save()
                print("⚠️ Created fallback block without full auto-programming")
                errorMessage = "Used fallback block because training maxes were invalid."
            }

            isGenerating = false
            dismiss()
        } catch {
            // Final fallback: if auto-programming failed for any reason, still create a basic block
            print("⚠️ AutoProgramService.generateBlock failed: \(error)")
            let fallback = BlockTemplate(
                name: trimmedName,
                weeksCount: weeks,
                createdAt: .now,
                createdByUser: true,
                notes: "Fallback block (auto-programming failed)"
            )
            context.insert(fallback)
            do {
                try context.save()
            } catch {
                print("🔥 Failed to save fallback block: \(error)")
            }
            isGenerating = false
            errorMessage = "Auto-programming failed; created a basic block instead."
            dismiss()
        }
    }
...
struct BlockDetailView: View {
    ...
    private func fetchSession(for day: DayTemplate) -> WorkoutSession? {
        do {
            ...
            print("Error fetching session in BlockDetailView: \(error)")
            return nil
        }
    }
